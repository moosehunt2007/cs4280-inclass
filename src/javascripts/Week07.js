...
export function displayCube() {
  ...
  function animate() {
    ...
    let t = new THREE.Matrix4().makeTranslation(.5, 0, .3)
    ...
    gl.uniformMatrix4fv(transformByLoc, false, new THREE.Matrix4().multiplyMatrices(q, new THREE.Matrix4().multiplyMatrices(s, e)).elements)
...
// 1/2 Sphere
export function displaySphereHalf() {
...
// Sphere
export function displaySphereOld() {
  const vs_script = `#version 300 es
    in vec3 coordinates;
    in vec3 color;
    out vec4 vColor;
